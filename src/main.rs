        ...
        process
            ...
            .map_err(|e| {
                Error::new(
                    ...
                    format!(
                        "[get_address_by_offsets]: Failed to read memory: {} {}",
                        e, offset
                    ),
                )
            })?;
...
fn main() {
    ...
    let localplayer_pointer_address: usize =
        match get_address_by_offsets(base_address as usize, vec![0x0018B0B8, 0x0], &process) {
            Ok(address) => address,
            Err(e) => {
                println!("{}", e);
                pause();
    ...
    let localplayer_address = localplayer_pointer_address + localplayer_offset;
